...
Workspaces have idle timeouts that stop workspaces that a user has not interacted with in a specified time. The idle timeout is set by the `CODENVY_MACHINE_WS_AGENT_INACTIVE_STOP_TIMEOUT_MS` property in the `codenvy.env` file with a default value of `600000` milliseconds or 10 minutes. This allows Codenvy to to free up unused resources quickly but may need to be increased if longer running workspaces are required.

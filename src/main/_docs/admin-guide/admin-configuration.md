...
Workspaces have idle timeouts that shutdown and remove workspaces that a user has not interacted within a specific amount of time set by `CODENVY_MACHINE_WS_AGENT_INACTIVE_STOP_TIMEOUT_MS` with a default value of `600000` milliseconds or 10 minutes. This allows Codenvy to to free up unused resources quickly but may need to be increased where longer running workspaces are required.

...
On codenvy.io, all users are granted 3 free GB of RAM which can be used either for personal workspaces or team workspaces. Additional RAM can be purchased to provide more resources for personal or team use. [TODO: add link to billing and subscriptions].
...
## Adding Team Members
Adding team members by clicking the "Add" button will display a new popup. You can add multiple users at the same time by separating emails with a comma but note that all users added at the same time will be given the same role [TODO: add link to roles section on this page]:
[TODO: create screenshot showing adding multliple users with the whole popup then remove the `team invte` and `team multiple invites` GIFs]
...
Click on a workspace to access its details or use it. Learn more about workspaces [TODO add link].
...
All members of a team are able to create workspaces in a team by clicking the "Add Workspaces" button. Team workspaces are administered in the same way as personal workspaces [TODO add link].
...
Note that when you are using a team, resources come from the team owner's account and may be subject to resource caps. [TODO add link].
...
As a **team owner or admin**, you are able to share workspaces with all members of the team by selecting the "share with all team members" option.

[TODO add snapshot when implem finished]

As a **team developer**, you can share workspaces with other team members by using the "Share" tab under the workspace's details. You can control which team members can access to the workspace.
...
Note that codenvy.io has a built-in workspace idle timeout of 10 minutes for free accounts (paid accounts receive a ... timeout). Locally installed Codenvy instances default to a 6 hour idle timeout, but this can be set in the `codenvy.env`.
